...
class CriticalRelationshipAdder {
  ...
  async addCriticalRelationships(): Promise<void> {
    ...
    const existingPairs = new Set(
      relationshipData.relationships.map(r => `${r.from}|${r.to}`)
    );
  ...
  private async generateContentForCriticalRelationships(relationships: CriticalRelationship[]): Promise<void> {
  ...
  private generateCriticalContent(rel: CriticalRelationship): any {
    ...
    if (rel.from === 'packages/core/src/runtime.ts' && rel.to === 'packages/core/src/types/index.ts') {
    ...
    } else if (rel.from === 'packages/server/src/index.ts' && rel.to === 'packages/server/src/api/index.ts') {
