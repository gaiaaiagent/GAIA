...
class EnhancedRelationshipAnalyzer {
  ...
  private async analyzePatternRelationships(): Promise<void> {
    ...
    const patterns = [
      ...
      { pattern: /types|interface/, relatedTo: /^((?!types|interface).)*$/, type: 'type-definition' },
  ...
  private async exportResults(): Promise<void> {
    ...
    console.log(chalk.gray(`  Strong (8-10): ${relationships.filter(r => r.strength >= 8).length}`));
    console.log(chalk.gray(`  Medium (5-7): ${relationships.filter(r => r.strength >= 5 && r.strength < 8).length}`));
...
async function main() {
  ...
  const scanPath = process.argv[2] || join(
    PROJECT_ROOT,
