...
class PsychologicalDepthEnhancer {
  ...
  private initializePatterns(): void {
    ...
    this.psychPatterns.set('runtime-types', {
      emotions: [
        'confidence when types match reality',
        'frustration when runtime errors contradict types',
        'relief when TypeScript catches issues early',
      ],
      scenarios: [
        'debugging a production issue at 2am',
        'onboarding a new team member',
        'refactoring legacy code',
      ],
      painPoints: [
        'type assertions that lie',
    ...
    this.psychPatterns.set('config-files', {
      emotions: [
        ...
        'fear of misconfiguration in production',
        'satisfaction when everything just works',
      ],
    ...
    this.psychPatterns.set('test-code', {
      emotions: [
        'confidence from green tests',
        'dread when tests are flaky',
        'satisfaction from high coverage',
      ],
      scenarios: [
    ...
    this.psychPatterns.set('documentation', {
      emotions: [
        'gratitude for clear docs',
        'frustration with outdated info',
        'pride in well-documented code',
      ],
      scenarios: [
  ...
  private needsEnhancement(psychological: string): boolean {
    ...
    const hasEmotions =
      /\b(frustrat|confus|relief|anxiety|confidence|pride|fear|joy|anger|stress)\b/i.test(
        psychological
      );

    // Check for lack of specific scenarios
    const hasScenarios =
      /\b(debug|onboard|refactor|deploy|review|implement|troubleshoot|maintain)\b/i.test(
        psychological
      );
  ...
  private getGenericEnhancement(): PsychologicalEnhancement {
    return {
      emotions: [
        ...
        'satisfaction when patterns emerge',
      ],
  ...
  private extractConcepts(text: string): string[] {
    ...
    const techMatches =
      text.match(/\b(import|export|config|api|server|client|database|runtime|types?)\b/gi) || [];
    concepts.push(...techMatches.map((m) => m.toLowerCase()));

    // Extract action concepts
    const actionMatches =
      text.match(/\b(debug|test|deploy|refactor|implement|maintain|review)\b/gi) || [];
  ...
  private hasEmotion(text: string): boolean {
    return /\b(frustrat|confus|relief|anxiety|confidence|pride|fear|joy|anger|stress|comfort|concern|satisf)\b/i.test(
      text
    );
  }

  private hasScenario(text: string): boolean {
    return /\b(when\s+\w+ing|during\s+\w+|while\s+\w+ing|at\s+\d+[ap]m|in\s+production|on\s+friday)\b/i.test(
      text
    );
  }
  ...
  private injectEmotion(text: string, emotion: string): string {
    ...
    if (trustMatch) {
      return text.replace(
        trustMatch[0],
        `developers experience ${emotion} and ${trustMatch[1]} this`
      );
    }
