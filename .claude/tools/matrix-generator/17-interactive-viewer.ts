...
class InteractiveMatrixViewer {
  ...
  private searchRelationships(): void {
    ...
    this.rl.question(chalk.yellow('Enter search term: '), (term) => {
      const results = this.contentData.relationshipCells.filter((cell) => {
        const text =
          `${cell.from} ${cell.to} ${cell.psychological} ${cell.technological} ${cell.thematic}`.toLowerCase();
  ...
  private showStatistics(): void {
    ...
    const stats = {
      ...
      weakRelationships: this.contentData.relationshipCells.filter((c) => c.metadata.strength <= 5)
        .length,
    };
  ...
  private showHubFiles(): void {
    ...
    hubs.forEach(([file, count], index) => {
      ...
      const outgoing = this.vizData.links.filter((l) => l.source === file).slice(0, 3);
