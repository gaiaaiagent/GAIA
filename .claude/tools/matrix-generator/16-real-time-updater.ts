...
class RealTimeMatrixUpdater {
  ...
  private async updateMatrix(changes: ChangeEvent[]): Promise<UpdateResult> {
    ...
    for (const rel of affectedRelationships) {
      ...
      if (existingIndex >= 0) {
        ...
        const updated = await this.regenerateContent(rel, contentData.relationshipCells[existingIndex]);
        if (updated) {
  ...
  private findAffectedRelationships(changedFiles: string[], relationships: any[]): any[] {
    ...
    for (const rel of relationships) {
      const isAffected = changedFiles.some(file => 
        rel.from.endsWith(file) || rel.to.endsWith(file)
      );
  ...
  private async regenerateContent(relationship: any, existing: any): Promise<any> {
    ...
    if (!updated.thematic.includes('[Updated]')) {
      updated.thematic += ' [Updated: This relationship may have evolved with recent code changes.]';
    }
  ...
  private displayStatus(): void {
    setInterval(() => {
      ...
      process.stdout.write(`\r${status} | Queue: ${this.changeQueue.length} | Last update: ${uptime}s ago`);
    }, 1000);
  }
